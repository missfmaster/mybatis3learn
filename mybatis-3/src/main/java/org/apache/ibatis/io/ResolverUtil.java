 ...
 *
 * <p>
 * General searches are initiated by calling the {@link #find(org.apache.ibatis.io.ResolverUtil.Test, String)} ()}
 * method and supplying a package name and a Test instance. This will cause the named package <b>and all
 * sub-packages</b> to be scanned for classes that meet the test. There are also utility methods for the common use
 * cases of scanning multiple packages for extensions of particular classes, or classes annotated with a specific
 * annotation.
 * </p>
 *
 * <p>
...
public class ResolverUtil<T> {
	...
	public interface Test {
		/**
		 * Will be called repeatedly with candidate classes. Must return True if a class is to be included in the results, false
		 * otherwise.
		 */
	...
	/**
	 * Attempts to discover classes that are assignable to the type provided. In the case that an interface is provided this
	 * method will collect implementations. In the case of a non-interface class, subclasses will be collected. Accumulated
	...
	/**
	 * Add the class designated by the fully qualified class name provided to the set of resolved classes if and only if it
	 * is approved by the Test supplied.
	 *
