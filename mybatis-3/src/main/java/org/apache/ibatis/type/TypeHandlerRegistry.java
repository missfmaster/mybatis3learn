...
/**
 * 一个容器，存储 java类型 -> jdbc类型 的转换器、jdbc类型 -> java类型 的转换器，
 * 并提供一系列的注册方法，可以根据注解的方式、继承抽象类的方式、扫描包的方式自我完善。
 * @see MappedTypes
 * @see MappedJdbcTypes
 * @see TypeReference
...
public final class TypeHandlerRegistry {
	...
	// 用于从结果集中读取数据时，将数据从jdbc类型转换为java类型
	private final Map<JdbcType, TypeHandler<?>> JDBC_TYPE_HANDLER_MAP = new EnumMap<JdbcType, TypeHandler<?>>(JdbcType.class);
	// 记录java类型转换为jdbc类型时用到的typehandler类
	...
	public <T> void register(TypeHandler<T> typeHandler) {
		...
		// 从3.1.0版本开始，可以根据TypeHandler自动查找java类型，但是需要继承TypeReference
		// 因为TypeReference存在方法可以查找对于的java类型
	...
	private void register(Type javaType, JdbcType jdbcType, TypeHandler<?> handler) {
		if (javaType != null) { // 如果明确指定了TypeHandler可以处理的java类型
			// 向java类型->jdbc类型转换的集合添加数据
